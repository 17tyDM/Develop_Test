...
for name, subjects in student_data.items():
    with open(f'生徒{int(name[-1])+1}.csv', 'w', newline='') as csvfile:
        ...
        for subject, scores in subjects.items():
            ...
            # 順位を計算
            ranked_students = sorted(student_data.keys(), key=lambda x: -sum(student_data[x][subject]))
            rank = ranked_students.index(name) + 1
